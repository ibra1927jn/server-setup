...
def main():
    ...
    run(
        "docker stop ultra_n8n 2>/dev/null; docker rm ultra_n8n 2>/dev/null",
        "Stop ultra_n8n",
    )
    run(
    ...
    run(
        "docker stop n8n-n8n-1 2>/dev/null; docker rm n8n-n8n-1 2>/dev/null",
        "Stop n8n-n8n-1",
    )
    ...
    code = run(
        "curl -s -o /dev/null -w '%{http_code}' http://localhost:5678/setup",
        "Setup page",
    )
