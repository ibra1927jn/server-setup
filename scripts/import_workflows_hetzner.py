...
def import_workflow(base_url, headers, workflow_data):
    ...
    resp = requests.post(f"{base_url}/api/v1/workflows", headers=headers, json=workflow_data, timeout=30)
...
def activate_workflow(base_url, headers, wf_id):
    ...
    resp = requests.patch(f"{base_url}/api/v1/workflows/{wf_id}", headers=headers, json={"active": True}, timeout=30)
