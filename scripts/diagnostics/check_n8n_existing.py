...
cmds = [
    ...
    ("ultra_n8n env vars", "docker inspect ultra_n8n --format '{{range .Config.Env}}{{println .}}{{end}}' 2>/dev/null | grep -i 'n8n\\|secure\\|cookie\\|webhook\\|auth' || echo 'No ultra_n8n'"),
    ("n8n container env", "docker inspect n8n --format '{{range .Config.Env}}{{println .}}{{end}}' 2>/dev/null | head -20 || echo 'No n8n container'"),
